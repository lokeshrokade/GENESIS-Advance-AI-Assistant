...
from Automation import ChromeAuto as AutoChromeAuto, YouTubeAuto as AutoYouTubeAuto
...
# Pick a sensible available voice (prefer female if present)
selected_voice = None
for v in voices:
    try:
...
def pearson(my_string):
    ...
    try:
        results = list(search(query, num_results=15, lang="en"))
    except TypeError:
...
def ChromeAuto(command):
    AutoChromeAuto(command)

def YouTubeAuto(command):
    AutoYouTubeAuto(command)
...
def webscrap_search(command):
    sounds()
    query = command
    # Try to strip leading verbs for better results
    for prefix in ["what is", "which is", "who is", "define", "meaning of"]:
    ...
    try:
        ...
        data = BeautifulSoup(r.text, "html.parser")
        text = None
        cand = data.find("div", class_="BNeawe")
        if cand:
            text = cand.get_text(strip=True)
        if not text:
            # Try description meta
            meta = data.find("meta", attrs={"name": "description"})
            if meta and meta.get("content"):
                text = meta.get("content").strip()
        if not text:
            # As a fallback, try Wikipedia summary
            text = wikipedia.summary(query, sentences=1)
        if text:
            notification("information", text)
            talk(text)
        else:
            talk("Opening results in browser")
    ...
    if 'play' in command:
        ...
        try:
            ...
            if chrome_path:
                ...
                # Fallback: open default browser to confirm action
...
def set_active_window(seconds: int = ACTIVE_LISTEN_SECONDS):
    global active_until_epoch
    try:
        active_until_epoch = int(time.time()) + int(seconds)
    except Exception:
        import time as _time
        active_until_epoch = int(_time.time()) + int(seconds)
...
while True:
    ...
    if now_epoch < active_until_epoch:
        run_genesis()
        continue

    wake_try = take_command()
    if is_wake_word(wake_try):
        sounds()
        talk("Yes, I'm listening")
        set_active_window()

# Lightweight HTTP API for Android app
app = Flask(__name__)
...
def http_wake():
    ...
    sounds()
    talk("Yes, I'm listening")
    return jsonify({"status": "ok", "active_for_seconds": ACTIVE_LISTEN_SECONDS})
